...
class AverageSalesCalculator:
    ...
    def calculate_sales_average(self, sender, instance) -> None:
        ...
        tank_volumes = (
            ...
            .values('id', 'volume', 'created_at')
            .order_by('created_at')
        )
        aggregated_volumes_by_date = groupby(
            tank_volumes, key=lambda x: (x['created_at'].strftime("%d %m %Y"))
        )
        ...
        for _, volume_agg in aggregated_volumes_by_date:
            ...
            combined = (
                list(zip(list_volume, list_volume[1:]))
                if len(list_volume) != two_pairs
                else zip(list_volume[::2], list_volume[1::2])
            )
            ...
            if not combined:
                ...
                if last_day_volume:
                    combined = [
                        (
                            {'volume': last_day_volume.volume},
                            {'volume': list_volume[0]['volume']},
                        )
                    ]
        ...
        if self.sales:
            ...
            self.save_average_sales(instance.tank, self.past_week_days[0], max_date)

    def get_max_date(self, created_at: timezone.datetime) -> timezone.datetime:
        """
    ...
    def create_past_weeks(
        self, date: timezone.datetime, end: int = 5
    ) -> List[timezone.datetime]:
        """
    ...
    def calculate_difference(self, combined_aggregation) -> None:
        """
    ...
    def save_average_sales(
        self, tank: int, date: timezone.datetime, max_date: timezone.datetime
    ) -> None:
        """
        ...
        if (
            all(d.date() in values for d in self.past_week_days)
            and len(values) >= past_week_number
        ):
            ...
            try:
                ...
                AverageSales.objects.create(
                    tank=tank, avg_sales=avg_sales, calculated_at=date
                )
            self.sales = []
